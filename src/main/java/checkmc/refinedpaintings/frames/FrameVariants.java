...
public class FrameVariants {
    ...
    private static final Map<String, checkmc.refinedpaintings.frames.Frame> frameDict = new HashMap<String, checkmc.refinedpaintings.frames.Frame>();
    ...
    public static void addNewFrame(Color color, Item item, String key) {
        checkmc.refinedpaintings.frames.Frame frame = new checkmc.refinedpaintings.frames.Frame(new ItemStack(item), color);
        frameDict.put(key, frame);
    }

    public static checkmc.refinedpaintings.frames.Frame getFrame(String key) {
        //PaintingFrames.LOGGER.info("Key "+key+" does not exist in frame dictionary.");
        return frameDict.get(key);
    }

    public static String getKey(checkmc.refinedpaintings.frames.Frame frame) {
    ...
    public static Frame frameFromItem(Item item) {
        for (checkmc.refinedpaintings.frames.Frame frame : frameDict.values()) {
